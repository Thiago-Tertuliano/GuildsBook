...
function convertGoogleBookToOurFormat(volume: GoogleBookVolume) {
  ...
  return {
    ...
    genre,
    publishedYear:
      publishedYear && publishedYear > 0 ? publishedYear : undefined,
    pages:
...
export async function syncGoogleBookToDatabase(
  bookData: ReturnType<typeof convertGoogleBookToOurFormat>
) {

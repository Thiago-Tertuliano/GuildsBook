...
export const userBookUpdateSchema = z
  .object({
    status: z.enum(["QUERO_LER", "LENDO", "LIDO"]).optional(),
    rating: z.number().int().min(1).max(5).optional(),
    review: z.string().optional(),
...
export const reviewUpdateSchema = z.object({
  content: z
    .string()
    .min(10, "A review deve ter pelo menos 10 caracteres")
    .optional(),
  rating: z.number().int().min(1).max(5).optional(),
});
...
export const quoteSchema = z.object({
  ...
  content: z
    .string()
    .min(1, "A citação não pode ser vazia")
    .max(2000, "A citação deve ter no máximo 2000 caracteres"),
...
export const bookClubSchema = z.object({
  name: z
    .string()
    .min(1, "Nome do clube é obrigatório")
  ...
  maxMembers: z
    ...
    .int()
    .positive()
    .max(1000, "Máximo de membros não pode exceder 1000")
    .optional(),
});

      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (
            ...
            try {
              const error = JSON.parse(responseText) as ApiErrorBody;
          ...
          if (
            safeErrorMessage &&
            typeof safeErrorMessage === "string" &&
          ...
          const errorMessageToUse: string = String(
            finalMessage || `Request failed with status ${response.status}`
          ).trim();

          throw new HttpError(errorMessageToUse, response.status);
        }
